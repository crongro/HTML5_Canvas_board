...
 * NHN_Library:Jindo-2.4.0;JavaScript Framework;
...
/**
	...
	$Jindo 객체는 프레임워크에 대한 정보와 유틸리티 함수를 제공한다.
...
/**
	...
	$Jindo() 객체를 생성한다. $Jindo 객체는 프레임웍에 대한 정보와 유틸리티 함수를 제공한다.
...
jindo.$Jindo._maxWarn = function(nCurrentLength, nMaxLength, sMessage) {
    if(nCurrentLength > nMaxLength) {
        jindo.$Jindo._warn('추가적인 파라메터가 있습니다. : '+sMessage);
    }
};
...
jindo.$Jindo.checkVarType._throwException = function(aArgs, oRules, sFuncName) {
    ...
    if (/(\$\w+)(#\w+)?/.test(sFuncName)) {
        sURL = 'http://jindo.nhncorp.com/docs/jindo/archive/Jindo2-2.4.0/desktop/ko/symbols/' + encodeURIComponent(RegExp.$1) + '.html' + RegExp.$2;
    }
...
var _getElementById = function(doc,id){
    ...
    if(typeof MSApp != "undefined"){
        MSApp.execUnsafeLocalFunction(function(){
            eDiv.innerHTML = "<input type='input' name='"+sCheckId+"'/>";
        });
    }else{
        eDiv.innerHTML = "<input type='input' name='"+sCheckId+"'/>";
    }
...
/**
	...
	$Class 객체는 Jindo 프레임워크를 사용하여 객체 지향 프로그래밍 방식으로 애플리케이션을 구현할 수 있도록 지원한다.
...
/**
	...
	클래스($Class() 객체)를 생성한다. 파라미터로 클래스화할 객체를 입력한다. 해당 객체에 $init 이름으로 메서드를 등록하면 클래스 인스턴스를 생성하는 생성자 함수를 정의할 수 있다. 또한  키워드를 사용하면 인스턴스를 생성하지 않아도 사용할 수 있는 메서드를 등록할 수 있다.
...
/**
	...
	@method kindOf
	@param {$Class} oClass 확인할 클래스($Class() 객체)
	@return {Boolean} true | false
	@since 2.0.0
	@example
		var Parent = $Class ({});
...
function _kindOf(oThis, oClass){
 ...
 /**
	...
	extend() 메서드는 특정 클래스($Class() 객체)를 상속한다. 상속할 부모 클래스(Super Class)를 지정한다.
...
결과 캐쉬를 사용중이면 캐쉬에 저장
 ...
 @lends $$
		...
		cssquery = function(sQuery, oParent, oOptions){
			...
			sTempId, aRet, nParentNodeType, sProperty, oOldParent, queryid, _clone, sTagName;
...
/**
	...
	$Agent 객체는 운영체제, 브라우저를 비롯한 사용자 시스템 정보를 제공한다.
...
/**
	...
	$Agent() 객체를 생성한다. $Agent() 객체는 사용자 시스템의 운영 체제 정보와 브라우저 정보를 제공한다.
...
/**
	...
	$A 객체는 배열(Array)을 좀 더 편리하게 다룰 수 있도록 메서드를 제공한다. $A() 객체를 생성할 때 원본 배열 객체를 래핑(warpping)하여 생성한다. 여기서 래핑이란 자바스크립트의 함수를 감싸 본래 함수의 기능에 새로운 확장 기능을 추가하는 것을 말한다.
...
/**
	...
	@return {$A} 삭제한 원소를 래핑하는 새로운 $A() 객체. 삭제한 원소가 없을 경우 빈 배열을 가진 $A() 객체를 반환한다.
	@example
...
/**
	...
	filter() 메서드는 배열의 모든 원소를 순회하면서 콜백 함수를 실행하고 콜백 함수가 true 값을 반환하는 원소만 모아 새로운 $A() 객체를 반환한다.
...
/**
	...
	refuse() 메서드는 배열에서 입력한 파라미터와 같은 값을 제외하여 새로운 $A() 객체를 생성한다. 제외할 값을 여러 개 지정할 수 있다.
...
/**
	...
	$Ajax 객체는 다양한 개발 환경에서 Ajax 요청과 응답을 쉽게 구현하기 위한 메서드를 제공한다.
...
/**
	...
	$Ajax() 객체는 서버와 브라우저 사이의 비동기 통신, 즉 Ajax 통신을 지원한다. $Ajax() 객체는 XHR 객체(XMLHTTPRequest)를 사용한 기본적인 방식과 함께 다른 도메인 사이의 통신을 위한 여러 방식을 제공한다.
	...
	@remark $Ajax() 객체의 기본적인 초기화 방식은 다음과 같다.
<pre class="code "><code class="prettyprint linenums">
	...
	oOption 객체의 프로퍼티와 사용법에 대한 설명은 다음 표와 같다.<br>
		<h5>oOption 객체의 속성</h5>
		<table class="tbl_board">
			<caption class="hide">oOption 객체의 속성</caption>
			<thead>
				<th scope="col" style="width:18%">속성</th>
				<th scope="col" style="width:12%">타입</th>
				<th scope="col">설명</th>
			</thead>
			<tbody>
				<tr>
					<td class="txt bold">type</td>
					<td class="txt">String</td>
					<td class="txt">
						Ajax 요청 방식. 생략하면 기본값은 xhr이다.
						<ul class="disc">
							<li><strong>xhr</strong><br>
								브라우저에 내장된 XMLHttpRequest 객체를 이용하여 Ajax 요청을 처리한다. 
								text, xml, json 형식의 응답 데이터를 처리할 수 있다. 
								요청 실패 시 HTTP 응답 코드를 통해 원인 파악이 가능하다.
								2.1.0이상에서는 크로스 도메인이 아닌 xhr의 경우 헤더에 "X-Requested-With" : "XMLHttpRequest"을 포함함. 
								단, 크로스 도메인(Cross-Domain) 상황에서 사용할 수 없다.
								※ 2.1.0이상은 모바일에서 가능. 반드시 서버설정이 필요. 자세한 사용법은 <auidoc:see content="http://devcafe.nhncorp.com/ajaxui/board_5/574863">devcafe</auidoc:see>를 참고)
							</li>
				...
				<tr>
					...
					<td class="txt">
						요청 타임 아웃 시간(초 단위).<br>
						기본값은 0으로 타임 아웃을 적용하지 않는다. 타임 아웃 시간 안에 요청이 완료되지 않으면 Ajax 요청을 중지한다. 비동기 호출인 경우에만 사용 가능하다.
					</td>
				</tr>
				<tr>
					<td class="txt bold">withCredentials</td>
					<td class="txt">Boolean</td>
					<td class="txt">
						xhr에서 크로스 도메인 사용할 때 쿠키 사용여부(초 단위). 모바일만 가능함.<br>
						기본값은 false이며 true로 설정하면 서버에서도  "Access-Control-Allow-Credentials: true" 헤더를 설정해야 함.
					</td>
				</tr>
				<tr>
					<td class="txt bold">onload</td>
					<td class="txt">Function</td>
					<td class="txt">
						요청이 완료되면 실행할 콜백 함수.<br>
						반드시 지정해야 하며 콜백 함수의 파라미터로 응답 객체인 <auidoc:see content="$Ajax.Response"/> 객체가 전달된다.
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="txt">
						콜백 함수 이름.<br>
						type 속성이 "jsonp"일 때 사용하며, 서버에 요청할 콜백 함수의 이름을 지정할 수 있다. 기본값은 "_callback"이다(1.3.8 부터 지원).
					</td>
				</tr>
				<tr>
					<td class="txt bold">sendheader</td>
					<td class="txt">Boolean</td>
					<td class="txt">
						요청 헤더를 전송할지 여부.<br>
						...
						allow-header가 설정되어 있지 않다면 반드시 false로 설정해야 한다. 기본값은 true 이다(1.3.4부터 지원). 
					</td>
				</tr>
				<tr>
					...
					<td class="txt">
						비동기 호출 여부.<br>
						type 속성이 "xhr"일 때 이 속성 값이 유효하다. 기본값은 true 이다(1.3.7부터 지원).
					</td>
				</tr>
				<tr>
					...
					<td class="txt">Boolean</td>
					<td class="txt">
						type 속성이 "flash"일 때 사용하며, 요청한 데이터 안에 utf-8 이 아닌 다른 인코딩이 되어 있을때 false 로 지정한다. 기본값은 true 이다(1.4.0부터 지원). 
					</td>
				</tr>
				<tr>
...
/**
	...
	request() 메서드는 Ajax 요청을 서버에 전송한다. 요청에 사용할 파라미터는 $Ajax() 객체 생성자에서 설정하거나 option() 메서드를 사용하여 변경할 수 있다. 
	요청 타입(type)이 "flash"면 이 메소드를 실행하기 전에 body 요소에서 <auidoc:see content="$Ajax.SWFRequest.write"/>() 메서드를 반드시 실행해야 한다.
...
/**
	...
	option() 메서드는 $Ajax() 객체의 옵션 객체(oOption) 속성에 정의된 Ajax 요청 옵션에 대한 정보를 가져온다.
...
/**
	...
	option() 메서드는 $Ajax() 객체의 옵션 객체(oOption) 속성에 정의된 Ajax 요청 옵션에 대한 정보를 설정한다. Ajax 요청 옵션을 설정하려면 이름과 값을, 혹은 이름과 값을 원소로 가지는 하나의 객체를 파라미터로 입력한다. 이름과 값을 원소로 가지는 객체를 입력하면 하나 이상의 정보를 한 번에 설정할 수 있다.
...
/**
	...
	$Ajax.Response 객체를 생성한다. $Ajax.Response 객체는 $Ajax() 객체에서 request() 메서드의 요청 처리 완료한 후 생성된다. $Ajax() 객체를 생성할 때 onload 속성에 설정한 콜백 함수의 파라미터로 $Ajax.Response 객체가 전달된다.
...
/**
	...
	@remark readyState 속성 값에 대한 설명은 다음 표와 같다.<br>
		<table class="tbl_board">
			<caption class="hide">readyState 속성 설명</caption>
			<thead>
				<tr>
					<th scope="col" style="width:20%">값</th>
					<th scope="col">설명</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="txt">0(UNINITIALIZED)</td>
					<td class="txt">요청이 초기화되지 않은 상태.</td>
				</tr>
				<tr>
					<td class="txt">1(LOADING)</td>
					<td class="txt">요청 옵션을 설정했으나, 요청하지 않은 상태</td>
				</tr>
				<tr>
					<td class="txt">2(LOADED)</td>
					<td class="txt">요청을 보내고 처리 중인 상태. 이 상태에서 응답 헤더를 얻을 수 있다.</td>
				</tr>
				<tr>
					<td class="txt">3(INTERACTIVE)</td>
					<td class="txt">요청이 처리 중이며, 부분적인 응답 데이터를 받은 상태.</td>
				</tr>
				<tr>
					<td class="txt">4(COMPLETED)</td>
					<td class="txt">응답 데이터를 모두 받아 통신을 완료한 상태.</td>
				</tr>
			</tbody>
		</table>
	@example
...
/**
	...
	Ajax 요청 타입이 jsonp인 요청 객체를 생성하며, $Ajax() 객체에서 Ajax 요청 객체를 생성할 때 사용한다.
	
	@class $Ajax.JSONPRequest
...
/**
	...
	$Ajax.JSONPRequest 객체를 생성한다. 이때, $Ajax.JSONPRequest 객체는 $Ajax.RequestBase 객체를 상속한다.
...
/**
	...
	Ajax 요청 타입이 flash인 요청 객체를 생성하며, $Ajax() 객체에서 Ajax 요청 객체를 생성할 때 사용한다.
	
	@class $Ajax.SWFRequest
	@extends $Ajax.RequestBase
	@filter desktop
...
/**
	...
	$Ajax.SWFRequest 객체를 생성한다. 이때, $Ajax.SWFRequest 객체는 $Ajax.RequestBase 객체를 상속한다.
...
/**
	...
	write() 메서드는 플래시 객체를 초기화하는 메서드로서 write() 메서드를 호출하면 통신을 위한 플래시 객체를 문서 내에 추가한다. Ajax 요청 타입이 flash이면 플래시 객체를 통해 통신한다. 따라서 $Ajax() 객체의 request 메서드가 호출되기 전에 write() 메서드를 반드시 한 번 실행해야 하며, <body> 요소에 작성되어야 한다. 두 번 이상 실행해도 문제가 발생한다.
...
/**
	...
	$Ajax.FrameRequest 객체는 Ajax 요청 타입이 iframe인 요청 객체를 생성하며, $Ajax() 객체에서 Ajax 요청 객체를 생성할 때 사용한다.
	
	@class $Ajax.FrameRequest
	@extends $Ajax.RequestBase
	@filter desktop
	@ignore
...
/**
	...
	$Ajax.FrameRequest 객체를 생성한다. 이때, $Ajax.FrameRequest 객체는 $Ajax.RequestBase 객체를 상속한다.
...
/**
	...
	$Ajax.Queue 객체는 Ajax 요청을 큐에 담아 큐에 들어온 순서대로 요청을 처리한다.
...
/**
	...
	$Ajax() 객체를 순서대로 호출할 수 있도록 기능을 제공한다.
...
/**
	...
	add() 메서드는 $Ajax.Queue에 Ajax 요청($Ajax() 객체)을 추가한다.
...
/**
	...
	$H 객체는 키(key)와 값(value)을 원소로 가지는 열거형 배열인 해시(Hash)를 구현하고, 해시를 다루기 위한 여러 가지 위한 메서드를 제공한다.
...
/**
	...
	forEach() 메서드는 해시의 모든 원소를 순회하면서 콜백 함수를 실행한다. 이때 해시 객체의 키와 값 그리고 원본 해시 객체가 콜백 함수의 파라미터로 입력된다. $A() 객체의 forEach() 메서드와 유사하다. $H.Break()와 $H.Continue()을 사용할 수 있다.
...
/**
	...
	filter() 메서드는 해시의 모든 원소를 순회하면서 콜백 함수를 실행하고 콜백 함수가 true 값을 반환하는 원소만 모아 새로운 $H() 객체를 반환한다. $A() 객체의 filter() 메서드와 유사하다. $H.Break()와 $H.Continue()을 사용할 수 있다.
...
/**
	...
	map() 메서드는 해시의 모든 원소를 순회하면서 콜백 함수를 실행하고 콜백 함수의 실행 결과를 배열의 원소에 설정한다. $A() 객체의 map() 메서드와 유사하다. $H.Break()와 $H.Continue()을 사용할 수 있다.
...
/**
	...
	$Json 객체는 JSON(JavaScript Object Notation)을 다루기 위한 다양한 기능을 제공한다. 생성자에 파라미터로 객체나 문자열을 입력한다. XML 형태의 문자열로 $Json() 객체를 생성하려면 fromXML() 메서드를 사용한다.
...
/**
	...
	fromXML() 메서드는 XML 형태의 문자열을 $Json() 객체로 인코딩한다. XML 형식의 문자열에 XML 요소가 속성을 포함하고 있을 경우 해당 요소의 정보에 해당하는 내용을 하위 객체로 표현한다. 이때 요소가 CDATA 값을 가질 경우 $cdata 속성으로 값을 저장한다.
...
/**
	...
	get() 메서드는 특정 경로(path)에 해당하는 $Json() 객체의 값을 반환한다.
...
/**
	...
	toString() 메서드는 $Json() 객체를 JSON 문자열 형태로 반환한다.
...
/**
	...
	toXML() 메서드는 $Json() 객체를 XML 형태의 문자열로 반환한다.
...
/**
	...
	toObject() 메서드는 $Json() 객체를 원래의 데이터 객체로 반환한다.
...
/**
	...
	$Cookie 객체는 쿠키(Cookie)에 정보를 추가, 수정, 혹은 삭제하거나 쿠키의 값을 가져온다.
...
/**
	...
	@constructor
	@param {Boolean} [bURIComponent] 값 인코드/디코드시 encodeURIComponent()/decodeURIComponent() 함수를 사용할지 여부. true 지정시 encodeURIComponent()/decodeURIComponent() 함수를 사용하며, false를 지정하거나 생략하면 escape()/unescape() 함수를 사용한다.
...
/**
	...
	@param {Numeric} [nDays] 쿠키 유효 시간. 유효 시간은 일단위로 설정한다. 유효시간을 생략하거나 0을 넣으면 쿠키는 웹 브라우저가 종료되면 없어진다.
...
/**
	...
	$Event() 객체는 Event 객체를 래핑하여 이벤트 처리와 관련된 확장 기능을 제공한다. 사용자는 $Event() 객체를 사용하여 발생한 이벤트에 대한 정보를 파악하거나 동작을 지정할 수 있다.
...
/**
	...
	@method mouse
	@param {Boolean} [bIsScrollbar=false] true이면 scroll속성을 알 수 있다.
	@return {Object} 마우스 이벤트 정보를 갖는 객체.
	@filter desktop
	@remark 2.0.0부터는 인자로 boolean값을 넣을 수 있으며 true인 경우 scroll속성을 알 수 있다.<br>
		마우스 이벤트 정보 객체의 속성에 대한 설명은 다음 표와 같다.<br>
		<table class="tbl_board">
			<caption class="hide">마우스 이벤트 정보 객체 속성</caption>
			<thead>
				<tr>
					<th scope="col" style="width:15%">속성</th>
					<th scope="col" style="width:15%">타입</th>
					<th scope="col">설명</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="txt bold">delta</td>
					<td>Number</td>
					<td class="txt">마우스 휠을 굴린 정도를 정수로 저장한다. 마우스 휠을 위로 굴린 정도는 양수 값으로, 아래로 굴린 정도는 음수 값으로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">left</td>
					<td>Boolean</td>
					<td class="txt">마우스 왼쪽 버튼 클릭 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">middle</td>
					<td>Boolean</td>
					<td class="txt">마우스 가운데 버튼 클릭 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">right</td>
					<td>Boolean</td>
					<td class="txt">마우스 오른쪽 버튼 클릭 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">scroll</td>
					<td>Boolean</td>
					<td class="txt">이벤트가 스크롤에서 발생했는지를 알수있다.</td>
				</tr>
			</tbody>
		</table>
	@example
...
/**
	...
	@remark 키보드 이벤트 정보 객체의 속성에 대한 설명은 다음 표와 같다.<br>
		<table class="tbl_board">
			<caption class="hide">키보드 이벤트 정보 객체 속성</caption>
			<thead>
				<tr>
					<th scope="col" style="width:15%">속성</th>
					<th scope="col" style="width:15%">타입</th>
			...
			<tbody>
				<tr>
					<td class="txt bold">alt</td>
					<td>Boolean</td>
					<td class="txt">ALT 키 입력 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">ctrl</td>
					<td>Boolean</td>
					<td class="txt">CTRL 키 입력 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">down</td>
					<td>Boolean</td>
					<td class="txt">아래쪽 방향키 입력 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">enter</td>
					<td>Boolean</td>
					<td class="txt">엔터(enter)키 입력 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">esc</td>
					<td>Boolean</td>
					<td class="txt">ESC 키 입력 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">keyCode</td>
					<td>Boolean</td>
					<td class="txt">입력한 키의 코드 값을 정수 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">left</td>
					<td>Boolean</td>
					<td class="txt">왼쪽 방향키 입력 여부를 불리언 형태 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">meta</td>
					<td>Boolean</td>
					<td class="txt">META키(Mac 용 키보드의 Command 키) 입력 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">right</td>
					<td>Boolean</td>
					<td class="txt">오른쪽 방향키 입력 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">shift</td>
					<td>Boolean</td>
					<td class="txt">Shift키 입력 여부를 불리언 형태로 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">up</td>
					<td>Boolean</td>
					<td class="txt">위쪽 방향키 입력 여부를 불리언 형태로 저장한다.</td>
				</tr>
			</tbody>
		</table>
	@example
...
/**
	...
	@remark 
		...
		키보드 커서 위치 정보 객체의 속성에 대한 설명은 다음 표와 같다.<br>
		<h5>마우서 커서 위치 정보 객체 속성</h5>
		<table class="tbl_board">
			<caption class="hide">마우서 커서 위치 정보 객체 속성</caption>
			<thead>
				<tr>
					<th scope="col" style="width:15%">속성</th>
					<th scope="col" style="width:15%">타입</th>
			...
			<tbody>
				<tr>
					<td class="txt bold">clientX</td>
					<td>Number</td>
					<td class="txt">화면을 기준으로 마우스 커서의 X좌표를 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">clientY</td>
					<td>Number</td>
					<td class="txt">화면을 기준으로 마우스 커서의 Y좌표를 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">offsetX</td>
					<td>Number</td>
					<td class="txt">DOM 요소를 기준으로 마우스 커서의 상대적인 X좌표를 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">offsetY</td>
					<td>Number</td>
					<td class="txt">DOM 요소를 기준으로 마우스 커서의 상대적인 Y좌표를 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">pageX</td>
					<td>Number</td>
					<td class="txt">문서를 기준으로 마우스 커서의 X 좌표를 저장한다. @deprecated</td>
				</tr>
				<tr>
					<td class="txt bold">pageY</td>
					<td>Number</td>
					<td class="txt">문서를 기준으로 마우스 커서의 Y좌표를 저장한다. @deprecated</td>
				</tr>
			</tbody>
		</table>
	@example
...
/**
	...
	@method stop
	@param {Numeric} [nCancelConstant=$Event.CANCEL_ALL] $Event() 객체의 상수. 지정한 상수에 따라 이벤트의 버블링과 기본 동작을 선택하여 중지시킨다. 
		$Event() 객체의 상수 값으로 CANCEL_ALL, CANCEL_BUBBLE, CANCEL_DEFAULT가 있으며, 기본값은 CANCEL_ALL이다(1.1.3 버전 이상).
...
/**
	...
	$Element() 객체는 HTML 요소를 래핑(wrapping)하며, 해당 요소를 좀 더 쉽게 다룰 수 있는 기능을 제공한다.
...
/**
	...
	@param {Hash+} oList 하나 이상의 CSS 속성과 값을 가지는 객체(Object) 또는 해시 객체($H() 객체).
...
/**
	...
	@param {Hash+} oList 하나 이상의 속성과 값을 가지는 객체(Object) 또는 해시 객체($H() 객체).
...
/**
	...
	@method className
	@return {String} 클래스 이름(String)을 반환. 하나 이상의 클래스가 지정된 경우 공백으로 구분된 문자열이 반환된다.
...
/**
	...
	toogleClass() 메서드는 HTML 요소에 파라미터로 지정한 클래스가 이미 적용되어 있으면 제거하고 만약 없으면 추가한다.<br>
...
/**
	...
	@method cssClass
	@param {String+} sName class명(String)
...
/**
	...
	@param {Hash+} oList 하나 이상의 속성명과 불린값을 가지는 객체(Object) 또는 해시 객체($H()객체).
...
jindo.$Element.eventManager = (function(){
	...
	@param {Function} fpCallback 이벤트 콜백함수
        ...
        revisionCallback : function(sType, sEvent, fpCallback){
            if((document.addEventListener !== undefined&&(sEvent=="mouseenter"||sEvent=="mouseleave")) || 
               (_JINDO_IS_IE&&(sType=="delegate")&&(sEvent=="mouseenter"||sEvent=="mouseleave"))){
...
/**
	...
	@method offset
	@return {Hash+} HTML 요소의 위치 값을 객체로 반환한다.
	@remark
		<ul class="disc">
		...
		다음은 위치 값을 반환할 때 반환되는 객체의 속성 설명이다.
		<h5>HTML 요소 위치 정보 객체의 속성</h5>
		<table class="tbl_board">
			<caption class="hide">HTML 요소 위치 정보 객체의 속성</caption>
			<thead>
				<tr>
					<th scope="col" style="width:15%">이름</th>
					<th scope="col" style="width:15%">타입</th>
			...
			<tbody>
				<tr>
					<td class="txt bold">top</td>
					<td>Number</td>
					<td class="txt">문서의 맨 위에서 HTML 요소의 윗 부분까지의 거리를 저장한다.</td>
				</tr>
				<tr>
					<td class="txt bold">left</td>
					<td>Number</td>
					<td class="txt">문서의 왼쪽 가장자리에서 HTML 요소의 왼쪽 가장자리까지의 거리를 저장한다.</td>
				</tr>
			</tbody>
		</table>
	@example
...
/**
	...
	@return {$Element} 위치 값이 변경된 $Element() 객체를 반환한다.
	@remark
		<ul class="disc">
			<li>위치를 결정하는 기준점은 브라우저가 페이지를 표시하는 화면의 왼쪽 위 모서리이다.</li>
			<li>HTML 요소가 보이는 상태(display)에서 적용해야 한다. 요소가 화면에 보이지 않으면 정상적으로 동작하지 않을 수 있다.</li>
...
/**
	...
	append() 메서드는 $Element() 객체에 있는 요소의 마지막 자식 노드로 파라미터로 지정한 HTML 요소를 배정한다.
	
	@method append
	@syntax sId
	@syntax vElement
	@param {String+} sId 마지막 자식 노드로 배정할 HTML 요소의 ID
	@param {Element+ | Node} vElement 마지막 자식 노드로 배정할 HTML 요소(Element) 또는 $Element() 객체를 파라미터로 지정할 수 있다.
	@return {this} 인스턴스 자신
	@see $Element#prepend
	@see $Element#before
	@see $Element#after
...
/** 
	...
	prepend() 메서드는 $Element() 객체에 있는 요소의 첫 번째 자식 노드로 파라미터로 지정한 HTML 요소를 배정한다.
	
	@method prepend
	@syntax sId
	@syntax vElement
	@param {String+} sId 첫 번째 자식 노드로 배정할 HTML 요소의 ID
	@param {Element+ | Node} vElement 첫 번째 자식 노드로 배정할 HTML 요소(Element) 또는 $Element() 객체를 파라미터로 지정할 수 있다.
	@return {this} 인스턴스 자신
	@see $Element#append
	@see $Element#before
	@see $Element#after
...
/**
	...
	replace() 메서드는 $Element() 객체 내부의 HTML 요소를 지정한 파라미터의 요소로 대체한다.
	
	@method replace
	@syntax sId
	@syntax vElement
	@param {String+} sId 대체할 HTML 요소의 ID
	@param {Element+ | Node} vElement 대체할 HTML 요소(Element) 또는 $Element() 객체를 파라미터로 지정할 수 있다.
	@return {this} 인스턴스 자신
	@example
		// ID가 sample1인 HTML 요소에서
...
/**
	...
	appendTo() 메서드는 $Element() 객체에 있는 요소를 파라미터로 지정한 요소의 마지막 자식 요소로 배정한다.
	
	@method appendTo
	@syntax sId
	@syntax vElement
	@param {String+} sId 마지막 자식 노드가 배정 될 HTML 요소의 ID
	@param {Element+ | Node} vElement 마지막 자식 노드가 배정 될 HTML 요소(Element) 또는 $Element() 객체를 파라미터로 지정할 수 있다.
	@return {this} 인스턴스 자신
	@see $Element#append
	@see $Element#prepend
	@see $Element#before
...
/**
	...
	prependTo() 메서드는 $Element() 객체에 있는 요소를 파라미터로 지정한 요소의 첫 번째 자식 노드로 배정한다.
	
	@method prependTo
	@syntax sId
	@syntax vElement
	@param {String+} sId 첫 번째 자식 노드가 배정 될 HTML 요소의 ID
	@param {Element+ | Node} vElement 첫 번째 자식 노드가 배정 될 HTML 요소(Element) 또는 $Element() 객체를 파라미터로 지정할 수 있다.
	@return {this} 인스턴스 자신
	@see $Element#append
	@see $Element#prepend
	@see $Element#before
...
/**
	...
	before() 메서드는 $Element() 객체에 있는 요소의 이전 형제 노드(previousSibling)로 파라미터로 지정한 요소를 배정한다.
	
	@method before
	@syntax sId
	@syntax vElement
	@param {String+} sId 이전 형제 노드로 배정할 HTML 요소의 ID
	@param {Element+ | Node} vElement 이전 형제 노드로 배정할 HTML 요소(Element) 또는 $Element() 객체를 파라미터로 지정할 수 있다.
	@return {this} 인스턴스 자신
	@see $Element#append
	@see $Element#prepend
	@see $Element#after
...
/**
	...
	after() 메서드는 $Element() 객체에 있는 요소의 다음 형제 노드(nextSibling)로 파라미터로 지정한 요소를 배정한다.
	
	@method after
	@syntax sId
	@syntax vElement
	@param {String+} sId 다음 형제 노드로 배정할 HTML 요소의 ID
	@param {Element+ | Node} vElement 다음 형제 노드로 배정할 HTML 요소(Element) 또는 $Element() 객체를 파라미터로 지정할 수 있다.
	@return {this} 인스턴스 자신
	@see $Element#append
	@see $Element#prepend
	@see $Element#before
...
/**
	...
	@method prev
	@param {Function+} [fCallback] 이전 형제 요소의 검색 조건을 지정한 콜백 함수.<br>파라미터로 콜백 함수를 지정하면 콜백 함수의 실행 결과가 true를 반환하는 이전 형제 요소를 반환한다. 이때 콜백 함수는 결과를 배열로 반환한다. 콜백 함수의 파라미터로 탐색 중인 이전 형제 요소의 $Element() 객체가 입력된다.
	@return {Variant} 조건을 만족하는 이전 형제 요소($Element() 객체)를 원소로 갖는 배열(Array).<br>fCallback이 null인 경우 모든 이전 형제 요소의 배열(Array)을 반환한다. 파라미터를 생략하면 바로 이전 형제 요소가 담긴 $Element() 객체. 만약 엘리먼트가 없으면 null을 반환한다.
	@see $Element#parent
	@see $Element#child
	@see $Element#next
	@see $Element#first
...
/**
	...
	@method next
	@param {Function+} [fCallback] 다음 형제 요소의 검색 조건을 지정한 콜백 함수.<br>파라미터로 콜백 함수를 지정하면 콜백 함수의 실행 결과가 true를 반환하는 다음 형제 요소를 반환한다. 이때 콜백 함수는 결과를 배열로 반환한다. 콜백 함수의 파라미터로 탐색 중인 다음 형제 요소의 $Element() 객체가 입력된다.
	@return {Variant} 조건을 만족하는 다음 형제 요소($Element() 객체)를 원소로 갖는 배열(Array).<br>fCallback이 null인 경우 모든 다음 형제 요소의 배열(Array)을 반환한다. 파라미터를 생략하면 바로 다음 형제 요소가 담긴 $Element() 객체. 만약 엘리먼트가 없으면 null을 반환한다.
	@see $Element#parent
	@see $Element#child
	@see $Element#prev
	@see $Element#first
...
/**
	...
	@return {Array} CSS 셀렉터 조건을 만족하는 HTML 요소($Element 객체)를 배열로 반환한다. 만족하는 HTML 요소가 존재하지 않으면 빈 배열을 반환한다.
	@see $Element#query
...
/**
	...
	@method query
	@param {String+} sSelector CSS 선택자. CSS 선택자로 사용할 수 있는 패턴은 표준 패턴과 비표준 패턴이 있다. 표준 패턴은 CSS Level3 명세서에 있는 패턴을 지원한다.
	@return {$Element} CSS 선택자의 조건을 만족하는 첫 번째 HTML 요소의 $Element인스턴스. 만족하는 HTML 요소가 존재하지 않으면 null을 반환한다.
	@see $Element#test
	@see $Element#queryAll
	@see http://www.w3.org/TR/css3-selectors/ CSS Level3 명세서 - W3C
	@example
...
/**
	...
	@method xpathAll
	@param {String+} sXPath XPath 값.
	@return {Array} XPath 문법을 만족하는 요소($Element 객체)를 원소로 하는 배열.
...
/**
	...
	@remark 1.4.8 버전부터 $Element 객체를 반환한다.
	@since 1.4.6
...
/**
	...
	@remark 1.4.8 부터 $Element 객체를 반환한다.
	@since 1.4.6
	@see $Element#appendHTML
	@see $Element#prependHTML
	@see $Element#afterHTML
...
/**
	...
	@param {String+} sHTML 삽입할 HTML 문자열.
	@return {$Element} HTML 코드를 변경한 $Element() 객체.
	@since 1.4.8 버전부터 $Element 객체를 반환한다.
	@since 1.4.6
	@see $Element#appendHTML
	@see $Element#prependHTML
	@see $Element#beforeHTML
...
/**
	...
	@method data
	@param {String+} sName dataset 이름(String)
...
/**
	...
	@param {Hash+} oList 하나 이상의 dataset과 값을 가지는 객체(Object) 또는 해시 객체($H() 객체).
		...
		jindo.$Element.prototype.data = function(sKey, vValue) {
			...
			switch(oArgs+""){
				case "g":
					...
					return (sVal!=null)? eval("("+this._element.getAttribute("data-"+sKey)+")") : null;
				case "s4var":
					var oData;
					if(isNull(vValue)){
				...
				case "s4obj":
					for(var i in sKey){
						if(isNull(sKey[i])){
							...
							sToStr = jindo.$Json._oldToString(sKey[i]);
							if(sToStr!=null){
								this._element.setAttribute("data-"+i, sToStr);
							}
						}
					}
					return this;
...
/**
	...
	$Fn() 객체는 Function 객체를 래핑(wrapping)하여 함수와 관련된 확장 기능을 제공한다.
...
/**
	...
	$Fn() 객체()를 생성한다. 생성자의 파라미터로 특정 함수를 지정할 수 있다. 이 때, 함수와 함께 this 키워드를 상황에 맞게 사용할 수 있도록 실행 문맥(Execution Context)을 함께 지정할 수 있다. 또한 생성자의 파라미터로 래핑할 함수의 파라미터와 몸체를 각각 입력하여 $Fn() 객체를 생성할 수 있다.
...
/**
	...
	@remark $Fn() 객체에 바인딩하여 사용할 때 함수의 반환 값이 false인 경우, 인터넷 익스플로러에서 기본 기능을 막기 때문에 사용하지 않도록 주의한다.<br>
...
/**
	...
	$ElementList 객체는 여러 개의 DOM 요소를 한 번에 다룰 수 있는 기능을 제공한다. $ElementList 객체는 DOM 요소를 배열 형태로 관리한다.
...
/**
	...
	$ElementList() 객체를 생성한다. $ElementList() 객체를 생성할 때 DOM 요소의 ID 또는 ID를 원소로 같은 배열, 혹은 CSS 선택자, $Element() 객체를 원소로 갖는 배열 등을 사용하여 $ElementList() 객체를 생성한다.
	...
	@param {Array+} aList 문서에서 DOM 요소의 ID, Element, $Element() 객체를 원소로 같는 배열을 입력한다.
	@example
...
/**
	...
	get() 메서드는 $ElementList() 객체의 내부 요소 중에서 지정한 인덱스에 해당하는 요소를 반환한다.
...
/**
	...
	getFirst() 메서드는 $ElementList() 객체의 첫 번째 요소를 반환한다. get() 메서드에 인덱스 값으로 0을 지정한 것과 동일하다.	
...
/**
	...
	getLast() 메서드는 $ElementList() 객체의 마지막 요소를 반환한다. get() 메서드에 인덱스 값으로 마지막 인덱스 번호를 지정한 것과 동일하다.
...
/**
	...
	length() 메서드는 $A() 객체의 length() 메서드를 이용하여 $ElementList() 객체의 크기를 반환한다.
	
	@method length
	@return {Number} 현재 $ElementList() 객체의 배열 크기(Number)를 반환한다.
...
/**
	...
	length() 메서드는 $A() 객체의 length() 메서드를 이용하여 $ElementList() 객체의 크기를 조정한다.
	
	@method length
	@syntax nLen
	@param {Numeric} nLen 지정할 배열의 크기. nLen이 기존 $ElementList() 객체의 배열 크기보다 작으면 nLen 번째 이후의 원소는 제거한다.
	
	@syntax nLen2, oValue
	@param {Numeric} nLen2 지정할 배열의 크기. nLen이 기존 $ElementList() 객체의 배열 크기보다 크면 추가된 배열의 공간에 oValue 파라미터의 값으로 채운다. nLen이 기존 $ElementList() 객체의 배열 크기보다 작으면 nLen 번째 이후의 원소는 제거한다.
	@param {Variant} oValue 새로운 원소를 추가할 때 사용할 초기 값.
	
	@return {$ElementList} 내부 배열을 변경한 $ElementList() 객체를 반환한다.
	@since 1.4.3
	@see $A#length
...
/**
	...
	@method $value
	@return {Array} $Element() 객체를 원소로 가지는 배열.
...
/**
	...
	@method show
	@param {String+} [sDisplay] display 속성에 지정할 값.<br>파라미터를 생략하면 태그별로 미리 지정된 기본값이 속성 값으로 설정된다. 미리 지정된 기본값이 없으면 "inline"으로 설정된다. 에러가 발생한 경우는 "block"으로 설정된다.
	@return {$ElementList} display 속성이 변경된 $ElementList() 객체.
	@remark 1.4.5 버전부터 sDisplay 파라미터를 사용할 수 있다.
	@see http://www.w3.org/TR/2008/REC-CSS2-20080411/visuren.html#display-prop display 속성 - W3C CSS2 Specification
	@see $Element#show
	@see $Element#hide
	@see $ElementList#hide
	@example
		// 화면에 보이도록 설정
...
/**
	...
	@method hide
	@return {$ElementList} display 속성이 none으로 변경된 $ElementList() 객체.
	@see http://www.w3.org/TR/2008/REC-CSS2-20080411/visuren.html#display-prop display 속성 - W3C CSS2 Specification
	@see $Element#show
	@see $ElementList#show
	@see $Element#hide
	@example
		// 화면에 보이지 않도록 설정
...
/**
	...
	@method addClass
	@param {String+} sClass 추가할 클래스 이름. 둘 이상의 클래스를 추가하려면 클래스 이름을 공백으로 구분하여 나열한다.
	@return {$ElementList} 지정한 클래스가 추가된 $ElementList() 객체.
	@see $Element#className
	@see $ElementList#className
	@see $Element#hasClass
	@see $Element#removeClass
	@see $ElementList#removeClass
	@see $Element#toggleClass
	@see $ElementList#toggleClass
	@example
		// 클래스 추가
...
/**
	...
	@method removeClass
	@param {String+} sClass 제거할 클래스 이름. 둘 이상의 클래스를 제거하려면 클래스 이름을 공백으로 구분하여 나열한다.
	@return {$ElementList} 지정한 클래스가 제거된 $ElementList() 객체.
	@see $Element#className
	@see $ElementList#className
	@see $Element#hasClass
	@see $Element#addClass
	@see $ElementList#addClass
	@see $Element#toggleClass
	@see $ElementList#toggleClass
	@example
		// 클래스 제거
...
/**
	...
	@param {String+} sClass 추가 혹은 제거할 클래스 이름1.
	@param {String+} [sClass2] 추가 혹은 제거할 클래스 이름2.
	@return {$ElementList} 클래스가 추가 혹은 제거된 현재의 $ElementList 객체
	@import core.$Element[hasClass,addClass,removeClass]
	@see $Element#className
	@see $ElementList#className
	@see $Element#hasClass
...
/**
	...
	@param {String+} sEvent 발생시킬 이벤트 이름. on 접두사는 생략한다.
	@param {Hash+} [oProps] 이벤트 객체의 속성을 지정한 객체. 이벤트를 발생시킬 때 속성을 설정할 수 있다.
	@return {$ElementList} 이벤트가 발생한 HTML 요소의 $ElementList() 객체.
	@remark 
		<ul class="disc">
			<li>1.4.1 버전부터 keyCode 값을 설정할 수 있다.</li>
			<li>WebKit 계열에서는 이벤트 객체의 keyCode가 읽기 전용(read-only)인 관계로 key 이벤트를 발생시킬 경우 keyCode 값을 설정할 수 없었다.</li>
...
/**
	...
	@method leave
	@return {$ElementList} 부모 요소에서 제거된 $ElementList() 객체.
	@see $Element#empty
	@see $ElementList#empty
	@see $Element#remove
	@example
		// 부모 요소 노드에서 제거
...
/**
	...
	@method empty
	@return {$ElementList} 자식 노드를 모두 제거한 $ElementList 객체.
	@see $Element#leave
	@see $ElementList#leave
	@see $Element#remove
	@example
		// 자식 노드를 모두 제거
...
/**
	...
	@param {Numeric} [nDuration] HTML 요소가 완전히 나타날 때까지 걸리는 시간. 단위는 초(second)이다.
	@param {Function} [fCallback] HTML 요소가 완전히 나타난 후에 실행할 콜백 함수.
	@return {$ElementList} 현재의 $ElementList() 객체
	@remark
		<ul class="disc">
			<li>인터넷 익스플로러 6 버전에서 filter를 사용하면서 해당 요소가 position 속성을 가지고 있으며 사라지는 문제가 있다. 이 경우에는 HTML 요소에 position 속성이 없어야 정상적으로 사용할 수 있다.</li>
			<li>Webkit 기반의 브라우저(Safari 5 버전 이상, Mobile Safari, Chrome, Mobile Webkit), Opear 10.60 버전 이상의 브라우저에서는 CSS3 transition 속성을 사용한다. 그 이외의 브라우저에서는 자바스크립트를 사용한다.</li>
...
/**
	...
	@method className
	@param {String+} sClass 설정할 클래스 이름. 하나 이상의 클래스를 지정하려면 공백으로 구분하여 지정할 클래스 이름을 나열한다.
	@return {$ElementList} 지정한 클래스가 반영된 $ElementList 객체가 반환된다.
	@throws {jindo.$Except.NOT_FOUND_ARGUMENT} 파라메터가 없는 경우.
	@see $Element#hasClass
	@see $Element#addClass
	@see $ElementList#addClass
...
/**
	...
	@method width
	@param {Numeric} nWidth	설정할 너비 값. 단위는 픽셀(px)이며 파라미터의 값은 숫자로 지정한다.
	@return {$ElementList} 너비 값이 반영된 $ElementList() 객체를 반환한다.
	@remark 브라우저마다 Box 모델의 크기 계산 방법이 다르므로 CSS의 width 속성 값과 width 메서드()의 반환 값은 서로 다를 수 있다.
	@see $Element#height
	@see $ElementList#height
	@example
		// HTML 요소에 너비 값을 설정
...
/**
	...
	@method height
	@param {Number} nHeight 설정할 높이 값. 단위는 픽셀(px)이며 파라미터의 값은 숫자로 지정한다.
	@return {$ElementList} 높이 값이 반영된 $ElementList() 객체를 반환한다.
	@remark 브라우저마다 Box 모델의 크기 계산 방법이 다르므로 CSS의 height 속성 값과 height() 메서드의 반환 값은 서로 다를 수 있다.
	@see $Element#width
	@see $ElementList#width
	@example
		// HTML 요소에 높이 값을 설정
...
/**
	...
	@method text
	@param {String+} sText 지정할 텍스트.
	@return {$ElementList} 지정한 값으로 설정된 $ElementList() 객체를 반환.
	@example
		// 텍스트 노드 값 설정
...
/**
	...
	@method html
	@param {String+} sHTML 내부 HTML 코드로 설정할 HTML 문자열.
	@return {$ElementList} 지정한 값으로 변경한 $ElementList() 객체를 반환.
	@remark IE8에서 colgroup의 col을 수정하려고 할 때 colgroup을 삭제하고 다시 만든 후 col을 추가해야 합니다.
	@see https://developer.mozilla.org/en/DOM/element.innerHTML element.innerHTML - MDN Docs
	@see $Element#outerHTML
	@example
...
/**
	...
	@param {String+ | Numeric} vValue CSS 속성에 설정할 값. 숫자(Number) 혹은 단위를 포함한 문자열(String)을 사용한다.
	@param {Hash+} oList 하나 이상의 CSS 속성과 값을 가지는 객체(Object) 또는 해시 객체($H() 객체).
	@return {$ElementList} CSS 속성 값을 반영한 $ElementList() 객체를 반환한다.
	@throws {jindo.$Except.NOT_USE_CSS} css을 사용할 수 없는 엘리먼트 일 때.
	@remark 
		<ul class="disc">
			<li>CSS 속성은 카멜 표기법(Camel Notation)을 사용한다. 예를 들면 border-width-bottom 속성은 borderWidthBottom으로 지정할 수 있다.</li>
...
/**
	...
	@param {Variant} vValue 속성에 설정할 값. 숫자(Number) 혹은 단위를 포함한 문자열(String)을 사용한다. 또한 속성의 값을 null로 설정하면 해당 HTML 속성을 삭제한다.
	@param {Hash+} oList 하나 이상의 속성과 값을 가지는 객체(Object) 또는 해시 객체($H() 객체).
	@return {$ElementList} 속성 값을 반영한 $ElementList() 객체를 반환한다.
	@throws {jindo.$Except.NOT_USE_CSS} sName은 문자,오브젝트 나 $Hash여야 한다.
	@remark 2.2.0 버전 부터 &lt;select&gt; 엘리먼트에 사용시, 옵션값을 설정할 수 있다.
	@see $Element#css
	@see $ElementList#css
...
/**
	...
	$S 객체는 String 객체를 래핑(wrapping)하여 문자열을 처리하기 위한 확장 기능을 제공한다.
...
/**
	...
	$value() 메서드는 $S() 객체가 감싸고 있던 원본 문자열(String 객체)을 반환한다. toString() 메서드와 같은 의미이다.
...
/**
	...
	toString() 메서드는 $S() 객체가 감싸고 있던 원본 문자열(String 객체)을 반환한다. $value() 메서드와 같은 의미이다.
...
/**
	...
	@return {$S} 해당 크기만큼 문자열을 잘라낸 $S() 객체
	@example
		// 문서가 euc-kr 환경임을 가정합니다.
		var str = "한글과 English가 섞인 문장...";
...
/**
	...
	$Document() 객체를 생성한다. 파라미터를 생략하면 기본값으로 현재 문서의 document 요소가 입력된다.
...
/**
	...
	@remark 다음은 문서의 가로, 세로 크기 정보를 담고 있는 객체의 속성을 설명한 표이다.<br>
		<table class="tbl_board">
			<caption class="hide">문서 크기 정보 객체</caption>
			<thead>
				<tr>
					<th scope="col" style="width:15%">이름</th>
					<th scope="col" style="width:20%">타입</th>
					<th scope="col">설명</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="txt bold">width</td>
					<td>Number</td>
					<td class="txt">문서의 가로 크기 정보. 단위는 픽셀(px)이다.</td>
				</tr>
				<tr>
					<td class="txt bold">height</td>
					<td>Number</td>
					<td class="txt">문서의 세로 크기 정보. 단위는 픽셀(px)이다.</td>
				</tr>
			</tbody>
		</table>
...
/**
	...
	@remark 다음은 스크롤바의 위치 정보를 담고 있는 객체의 속성을 설명한 표이다.<br>
		<table class="tbl_board">
			<caption class="hide">스크롤바 위치 정보 객체의 속성</caption>
			<thead>
				<tr>
					<th scope="col" style="width:15%">이름</th>
					<th scope="col" style="width:20%">타입</th>
			...
			<tbody>
				<tr>
					<td class="txt bold">left</td>
					<td>number</td>
					<td class="txt">수평 스크롤바의 가로 위치. 단위는 픽셀(px)이다.</td>
				</tr>
				<tr>
					<td class="txt bold">height</td>
					<td>number</td>
					<td class="txt">수직 스크롤바의 세로 위치. 단위는 픽셀(px)이다.</td>
				</tr>
			</tbody>
		</table>
	@since 1.3.5
...
/**
	...
	@remark 다음은 화면에 보이는 영역의 가로, 세로 크기 정보를 담고 있는 객체의 속성을 설명한 표이다.<br>
		<table class="tbl_board">
			<caption class="hide">화면 크기 정보 객체</caption>
			<thead>
				<tr>
					<th scope="col" style="width:15%">이름</th>
					<th scope="col" style="width:20%">속성</th>
					<th scope="col">설명</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="txt bold">width</td>
					<td>number</td>
					<td class="txt">화면에 보이는 부분의 가로 크기. 단위는 픽셀(px)이다.</td>
				</tr>
				<tr>
					<td class="txt bold">height</td>
					<td>number</td>
					<td class="txt">화면에 보이는 부분의 세로 크기. 단위는 픽셀(px)이다.</td>
				</tr>
			</tbody>
		</table>
...
/**
	...
	@filter desktop
	@remark 이 메서드가 확인할 수 있는 렌더링 방식은 다음 표와 같다.<br>
		<table class="tbl_board">
			<caption class="hide">렌더링 방식</caption>
			<thead>
				<tr>
					<th scope="col" style="width:15%">렌더링 모드</th>
					<th scope="col">설명</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="txt bold">Standards</td>
					<td class="txt">표준 렌더링 모드.</td>
				</tr>
				<tr>
					<td class="txt bold">Almost</td>
					<td class="txt">유사 표준 렌더링 모드. 인터넷 익스플로러 외의 브라우저에서 DTD를 올바르게 지정하지 않았을 때 반환된다.</td>
				</tr>
				<tr>
					<td class="txt bold">Quirks</td>
					<td class="txt">비표준 렌더링 모드.</td>
				</tr>
			</tbody>
		</table>
...
/**
	...
	$Form 객체는 &lt;form&gt; 요소와 자식 요소를 제어하는 기능을 제공한다.
...
/**
	...
	@param {String} sId	&lt;form&gt; 요소의 ID(String). 만약 동일한 ID를 사용한 &lt;form&gt; 요소가 둘 이상이면 먼저 나오는 요소를 감싼 $Form() 객체를 반환한다.
...
/**
	...
	@param {Hash+} oList 하나 이상의 입력 요소 이름과 값을 지정한 객체(Object) 또는 해시 객체($H() 객체)
...
/**
	...
	$Template 객체를 사용하면 템플릿을 해석하여 템플릿에 동적으로 데이터를 적용할 수 있다.
...
/**
	...
	@param {String+} sTemplate 템플릿으로 사용할 문자열 또는 템플릿 HTML 요소의 id. 파라미터가 문자열이면 두 가지 방식으로 동작한다.
		<ul class="disc">
	...
	@param {String+} [sEngineName="default"] 사용할 컴파일 엔진의 이름.
	@remark 정적 메서드 jindo.$Template.addEngine()를 사용하여 사용자가 원하는 엔진을 등록하여 사용할 수 있다. 각 엔진별로 그에 상응하는 문법의 템플릿을 사용해야 하며, 사용가능한 컴파일 엔진의 종류는 다음 표를 참고한다.
		<h5>사용가능한 컴파일 엔진의 종류</h5>
		<table class="tbl_board">
			<caption class="hide">사용가능한 컴파일 엔진의 종류</caption>
			<thead>
				<tr>
					<th scope="col" style="width:15%">엔진</th>
			...
			<tbody>
				<tr>
					<td class="txt bold">default</td>
					<td class="txt">기존 $Template 엔진</td>
					<td class="txt">sEngineName 파라미터 생략시 "default" 엔진을 사용한다.</td>
				</tr>
				<tr>
					<td class="txt bold">micro</td>
					<td class="txt">John Resig의 <auidoc:see content="http://ejohn.org/blog/javascript-micro-templating/">Micro-Templating 엔진</auidoc:see></td>
					<td class="txt"></td>
				</tr>
				<tr>
					<td class="txt bold">handlebars</td>
					<td class="txt"><auidoc:see content="http://handlebarsjs.com/">handlebars 엔진</auidoc:see></td>
					<td class="txt">handlebars 엔진을 사용하기 위해서는 별도로 해당 js 파일을 삽입해야 한다. 삽입하지 않고 handlebars 엔진으로 인스턴스 생성 후, $Template#process 메서드를 호출 하면 jindo.$Except.NOT_FOUND_HANDLEBARS 오류가 발생한다.</td>
				</tr>
				<tr>
					<td class="txt bold">simple</td>
					<td class="txt">2.4.0 버전에 새롭게 추가된, 단순 문자열 치환 엔진</td>
					<td class="txt">simple 엔진의 템플릿 문법은 치환하고자 하는 문자열을 중괄호 2개로 감싸주면 된다. if 문이나 for 문등은 지원하지 않는다. ex) {{replaced}}</td>
				</tr>
			</tbody>
		</table>
	@see $Template.addEngine
...
/**
	...
	$Date() 객체는 Date 객체를 래핑(Wrapping)하여 날짜 및 시간을 처리하기 위한 확장 기능을 제공한다.
...
/**
	...
	$Date() 객체를 생성한다. $Date() 객체를 생성할 때 파라미터 없이 생성하거나 ISO Date 포맷 또는 밀리 초 단위의 정수를 생성자의 인수로 입력할 수 있다. ISO Date 포맷 문자를 넣은 경우 $Date.utc 속성을 기반하여 날짜를 계산한다.
	...
	@param {Date+} oDate Date 또는 $Date 객체
...
/**
	...
	names 속성은 $Date() 객체에서 사용하는 달, 요일, 오전/오후를 표시하는 문자열을 저장한다. s_ 를 접두어로 가지는 이름은 약어(abbreviation)이다.
...
/**
	...
	@param {Hash+} oNames 하나 이상의 names속성과 값을 가지는 객체(Object) 또는 해시 객체($H()객체).
...
/**
	...
	parse() 메서드는 인수로 지정한 문자열을 파싱하여 $Date() 객체를 생성한다.
...
/**
	...
	$value() 메서드는 $Date() 객체가 감싸고 있던 원본 Date 객체를 반환한다.
...
/**
	...
	format() 메서드는 $Date() 객체가 저장하고 있는 시간을 파라미터로 지정한 형식 문자열(Format Specifier)에 맞추어 변환한다.
...
/**
	...
	time() 메서드는 $Date() 객체가 가지고 있는 GMT(1970/01/01 00:00:00)를 기준으로 현재 시간까지 경과한 시간 값을 가져온다.
	
	@method time
	@return {Number} $Date() 객체가 저장하고 있는 밀리세컨드 단위의 timeStamp 값.
...
/**
	...
	time() 메서드는 GMT(1970/01/01 00:00:00)를 기준으로 경과한 시간을 $Date() 객체에 설정한다.
...
/**
	...
	year() 메서드는 $Date() 객체가 저장하고 있는 시각의 연도(year)를 가져온다.
	
	@method year
	@return {Number} $Date() 객체가 저장하고 있는 시각의 연도를 반환한다. 반환되는 연도 값의 형식은 4자리 정수이다.
...
/**
	...
	year() 메서드는 $Date() 객체의 연도(year)를 지정한 값으로 설정한다.
	
	@method year
	@param {Numeric} nYear $Date() 객체에 설정할 연도(year). 연도 값의 형식은 4자리 정수로 지정한다.
...
/**
	...
	month() 메서드는 $Date() 객체가 저장하고 있는 시각의 달(month)을 가져온다.
	
	@method month
	@return {Number} $Date() 객체가 저장하고 있는 시각의 달을 반환한다. 반환되는 달의 값은 0(1월)에서 11(12월)사이의 정수이다.
...
/**
	...
	month() 메서드는 $Date() 객체의 달(month)을 지정한 값으로 설정한다.
	
	@method month
	@param {Numeric} nMon $Date() 객체에 설정할 달(month). 달의 값은 0(1월)에서 11(12월)사이의 정수로 지정한다.
...
/**
	...
	date() 메서드는 $Date() 객체가 저장하고 있는 시각의 날짜(day of the month)를 가져온다.
	
	@method date
	@return {Number} $Date() 객체가 저장하고 있는 시각의 날짜를 반환한다. 반환되는 날짜의 값은 1에서 31 사이의 정수이다.
...
/**
	...
	date() 메서드는 $Date() 객체의 날짜(day of the month)를 지정한 값으로 설정한다.
	
	@method date
	@param {Numeric} nDate $Date() 객체에 설정할 날짜(day of the month). 날짜의 값은 1에서 31 사이의 정수로 지정한다.
...
/**
	...
	day() 메서드는 $Date() 객체가 지정하고 있는 시각의 요일(day of the week)를 가져온다.
	
	@method day
	@return {Number} $Date() 객체가 저장하고 있는 시각의 요일을 반환한다. 반환되는 요일의 값은 0(일요일)에서 6(토요일) 사이의 정수이다.
	@throws {jindo.$Except.INVALID_DATE} 파라메터나 숫자가 아닌 경우 예외상황 발생.
	@see $Date#Date
...
/**
	...
	hours() 메서드는 $Date() 객체가 저장하고 있는 시각의 시간(hour)을 가져온다.
	
	@method hours
	@return {Number} $Date() 객체가 저장하고 있는 시각의 시간를 반환한다. 반환되는 시간의 값은 0에서 23 사이의 정수이다.
...
/**
	...
	hours() 메서드는 $Date() 객체의 시간(hour)을 지정한 값으로 설정한다.
	
	@method hours
	@param {Numeric} nHour  $Date() 객체에 설정할 시간(hour). 시간의 값은 0에서 23 사이의 정수로 지정한다.
...
/**
	...
	minutes() 메서드는 $Date() 객체가 저장하고 있는 시각의 분(minute)을 가져온다.
	
	@method minutes
	@return {Number} $Date() 객체가 저장하고 있는 시각의 분을 반환한다. 반환되는 분의 값은 0에서 59 사이의 정수이다.
...
/**
	...
	minutes() 메서드는 $Date() 객체의 분(minute)을 지정한 값으로 설정한다. 
	
	@method minutes
	@param {Numeric} nMin $Date() 객체에 설정할 분(minute). 분의 값은 0에서 59 사이의 정수로 지정한다.
...
/**
	...
	seconds() 메서드는 $Date() 객체가 저장하고 있는 시각의 초(second)를 가져온다.
	
	@method seconds
	@return {Number} $Date() 객체가 저장하고 있는 시각의 초를 반환한다. 반환되는 초의 값은 0에서 59 사이의 정수이다.
...
/**
	...
	seconds() 메서드는 $Date() 객체의 초(second)를 지정한 값으로 설정한다.
	
	@method seconds
	@param {Numeric} nSec $Date() 객체에 설정할 초(second). 초의 값은 0에서 59 사이의 정수로 지정한다.
...
/**
	...
	isLeapYear() 메서드는 $Date() 객체가 저장하고 있는 시각의 연도가 윤년인지 검사한다.
...
/**
	...
	@param {String+} [sOption] 비교할 단위를 지정.(기본값은 천 분의 1초까지 비교하며, y, m, d, h, i, s등을 넣을 수 있다. 각 옵션은 y는 년, m은 달, d는 날, h는 시, i는 분, s는 초로 한다)
...
/**
	...
	$Window 객체는 브라우저가 제공하는 window 객체를 래핑하고, 이를 다루기 위한 여러가지 메서드를 제공한다.
...
/**
	...
	resizeTo() 메서드는 창의 크기를 지정한 크기로 변경한다. 지정한 크기는 프레임을 포함한 창 전체의 크기를 의미한다. 따라서 실제로 표현하는 컨텐트 사이즈는 브라우저 종류와 설정에 따라 달라질 수 있다. 브라우저에 따라 보안 문제 때문에, 창 크기가 화면의 가시 영역보다 커지지 못하도록 막는 경우도 있다. 이 경우에는 지정한 크기보다 창이 작아질 수 있다. 단위는 픽셀(px) 단위이다.
